...
public class IterativeDeepening_PVS extends Player {
    ...
    public MoveData move(GameEnvironment gameState) throws Exception {
        ...
        IterativeDeepening_PVS_Thread thread;
        if (isLimitTime) {
            thread = new IterativeDeepening_PVS_Thread(Integer.MAX_VALUE - 10, game, onlyCloseMoves);
        } else {
            thread = new IterativeDeepening_PVS_Thread(simulationLimit, game, onlyCloseMoves);
        }
        ...
        MoveData moveData = new MoveData(endTimestamp - startTimestamp, results.get("moveCount"),
                ...
                GraphLayout.parseInstance(this).totalSize() + GraphLayout.parseInstance(largestTT).totalSize()
                        + GraphLayout.parseInstance(previousScores).totalSize(),
